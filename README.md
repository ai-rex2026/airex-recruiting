...
`/start`（収集センター）が**標準の入口**です。ルート `/` およびログイン後は `/start` に遷移します。
ナビゲーションは業務フロー順（① 集める → ② 打診する → ③ 返信・掲載 → ④ 台帳・管理）にグループ化されています。
また、全画面の上部に**業務フローバー**（① 集める → ② 打診する → ③ 返信・交渉 → ④ 掲載・報告）を表示し、現在地のハイライトと各段階で対応待ちの件数（＋例外件数）を常に確認できます。

| パス | 役割 |
|---|---|
| `/start` | **収集センター（標準の入口）**。かんたん開始ウィザード＋案件×KWごとの収集状況一覧・再開・再収集 |
