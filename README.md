...
貼り付け収集（`/collect`）は収集状況タブ・/start の各KW行の「貼り付けで取り込む」から使います。
...
| パス | 役割 |
|---|---|
| `/start` | **案件・収集（標準の入口）**。かんたん開始ウィザード＋案件×KWごとの収集状況一覧・再開・再収集 |
| `/campaigns/[id]` | **案件詳細ハブ**。タブ：収集状況／陣取りボード／打診・返信・掲載／案件情報・KW |
| `/queue` | **承認キュー（中核）**。3タブ：承認待ち（送信承認／返信の分類）／例外対応／送信ログ |
| `/board` | 陣取りボード。**KW×メディア×1位〜5位の固定枠グリッド**（自社の陣は背景色・横スクロール）＋検索結果に紐づかない候補メディア |
...
収集は `collection_jobs` テーブルでジョブとして管理され、/start に**案件×キーワードごとの収集状況一覧**
（✅完了／⏳実行中／◻未実行／❌失敗）が表示されます。ブラウザを閉じても状態は残り、
「未実行を再開」「全KWを再収集」「（KW単位の）収集／再収集」から続行できます。
完了済みKWの再収集は新しいジョブ・新しいスナップショットとして履歴に積まれます（定点観測）。
